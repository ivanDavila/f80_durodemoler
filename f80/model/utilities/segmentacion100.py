...
i = 0
imgScale = 1/2
def segmentacionGrande(imagReal, imageColor, binImage):
    ...
    #IMMMMMASHOOOW---------------------------------------------------------
    # segmn_k = np.zeros([height,width], dtype=np.uint8)

    # for labels in np.unique(markers):
    #     color = list(np.random.choice(range(256), size=1))
    #     segmn_k[markers == labels] = color

    # vis_markers = cv.applyColorMap(segmn_k, cv.COLORMAP_JET)
    # height, width, _ = vis_markers.shape
    # th2 =  cv.resize(vis_markers, (int(width*imgScale),int(height*imgScale)))
    # cv.imshow("image_bin", th2)
    #-----------------------------------------------------------------------
    ...
    # segmn[image_bin_not == 255] = 0
        
        # if labels != 1:
        #     imagReal[markers == labels] = color
    ...
    # th2 =  cv.resize(markers, (int(width_im*imgScale),int(heigth_im*imgScale)))
    # cv.imshow("hello", th2)
    # cv.waitKey()
    return segmn
...
if __name__ == '__main__':
    ...
    segm = segmentacionGrande(image_prcess, image_segm, image_tresh)
